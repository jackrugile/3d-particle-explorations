	...
	_createClass(System, [{
		...
		value: function update() {
			...
			for (var _i = 0, l = this.particles.length; _i < l; _i++) {
				...
				for (var _j = _i + 1; _j < l; _j++) {
					var c2 = this.particles[_j];
					var c2pos = c2.mesh.position;
					var dx = c1pos.x - c2pos.x;
					var dy = c1pos.y - c2pos.y;
					var dist = dx * dx + dy * dy - 6;
					var radii = (c1.size + c2.size) * (c1.size + c2.size);
					if (dist < radii) {
						var angle = Math.atan2(dy, dx) + this.calc.rand(-0.05, 0.05);
						var diff = radii - dist;
